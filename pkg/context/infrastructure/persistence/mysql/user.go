...
func (db *UserTable) Update(user *user.User) error {
	result := db.table.Model(&User{}).Where(&User{Name: user.Name.Value}).Updates(user.ToPrimitives())
...
func (db *UserTable) Delete(name *user.Name) error {
	result := db.table.Model(&User{}).Where(&User{Name: name.Value}).Delete(&User{})
...
func (db *UserTable) Search(name *user.Name) (*user.User, error) {
	...
	result := db.table.Model(&User{}).Where(&User{Name: name.Value}).Scan(&primitive)
...
func NewUserTable(mdb *MySQL) (model.UserRepository, error) {
	err := mdb.Client.AutoMigrate(&User{})
